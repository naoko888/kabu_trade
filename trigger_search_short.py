...
# トリガー候補
# =========================================
def setup_short_mask(df):
    return (
        ((df["close"] >= df["bb_upper_3"]) & (df["vol_ratio"] >= 1.5)) |
        ((df["close"] >= df["bb_upper_3"]) & (df["rsi14"] >= 70)) |
        ((df["vol_ratio"] >= 1.5) & (df["ma50_slope"] < 0)) |
        ((df["close"] >= df["bb_upper_2"]) & (df["vol_ratio"] >= 1.5)) |
        ((df["rsi14"] >= 70) & (df["vol_ratio"] >= 1.5)) |
        ((df["close"] >= df["bb_upper_3"]) & (df["ma50_slope"] < 0)) |
        ((df["close"] >= df["bb_upper_2"]) & (df["rsi14"] >= 70))
    ).fillna(False)

def signal_short(df, i):
    return bool(setup_short_mask(df).iloc[i])
...
def trigger_topfail_break(df: pd.DataFrame, i: int) -> bool:
    if i < 3:
        return False
    ...
    recent_high = max(prev1["high"], prev2["high"], prev3["high"])
    recent_low = min(prev1["low"], prev2["low"], prev3["low"])

    high_fail = cur["high"] <= recent_high
    break_low = cur["close"] < recent_low

    return high_fail and break_low


def trigger_break_retest_fail(df: pd.DataFrame, i: int) -> bool:
    if i < 4:
        return False
    ...
    support_low = min(b1["low"], b2["low"])
    resistance_high = max(b1["high"], b2["high"], b3["high"], b4["high"])

    cond1 = b3["close"] < support_low
    cond2 = b4["high"] < resistance_high
    cond3 = b4["close"] <= b4["open"] or (b4["upper_wick"] > b4["body"])
    cond4 = cur["close"] < b3["low"]
...
def run_backtest_short_trigger(df, trigger_func, trigger_name):
    trades = []

    setup_mask = setup_short_mask(df)
...
def run_trigger_search(df: pd.DataFrame):
    trigger_map = {
        ...
        "topfail_break": trigger_topfail_break,          # A
        "break_retest_fail": trigger_break_retest_fail,  # B
    }
...
def main():
    ...
    df = add_targets(df, horizon=HORIZON, down_th=DOWN_TH)

    need_cols = [
        "ma20", "ma50", "dist_ma20_pct", "vol_ratio",
        "atr_ratio", "bb_upper_2", "bb_upper_3",
        "rsi14", "macd", "macd_signal", "macd_hist",
        "stoch_k", "stoch_d", "cci20", "roc5", "gap_pct"
    ]
    df = df.dropna(subset=need_cols).reset_index(drop=True)

    result = run_trigger_search(df)
    print(result)

    save_path = Path(r"C:\kabu_trade\trigger_search_short.csv")
    result.to_csv(save_path, index=False, encoding="utf-8-sig")
    print(f"保存: {save_path}")

    # ================================
    # break_retest_fail の詳細検証
    # ================================
    setup_mask = setup_short_mask(df)
    trigger_mask = df.index.to_series().apply(lambda i: trigger_break_retest_fail(df, i))
    cond = setup_mask & trigger_mask
    ...
    down_hit = (future_low - df["close"]) <= -DOWN_TH
    up_hit = (future_high - df["close"]) >= DOWN_TH

    # ================================
    # 条件発生回数 / 下げ回数 / 上げ回数
    # ================================
    total = int(cond.sum())
    down_count = int((cond & down_hit).sum())
    up_count = int((cond & up_hit).sum())

    print("\n===== 条件検証 =====")
    print(f"条件発生回数: {total}")
    print(f"下げた回数: {down_count}")
    print(f"上がった回数: {up_count}")

    # ================================
    # 上がった時の特徴を見る
    # ================================
    up_cases = df.loc[cond & up_hit, [
        "datetime", "close", "volume", "vol_ratio",
        "rsi14", "ma20_slope", "ma50_slope",
        "upper_wick_ratio", "lower_wick_ratio",
        "body_ratio", "dist_ma20_pct", "dist_ma50_pct"
    ]].copy()

    up_cases_path = Path(r"C:\kabu_trade\up_cases_break_retest_fail.csv")
    up_cases.to_csv(up_cases_path, index=False, encoding="utf-8-sig")
    print(f"上がったケース保存: {up_cases_path}")

    if len(up_cases) > 0:
        print("\n===== 上がった時の平均値 =====")
        print(up_cases.drop(columns=["datetime"]).mean(numeric_only=True))
    ...
    down_df = df.loc[cond & down_hit, cols].copy()
    up_df = df.loc[cond & up_hit, cols].copy()

    print("\n===== 比較 =====")

    if len(down_df) > 0:
        print("\n▼下げた時 平均")
        print(down_df.mean(numeric_only=True))

    if len(up_df) > 0:
        print("\n▼上がった時 平均")
        print(up_df.mean(numeric_only=True))

    if len(down_df) > 0 and len(up_df) > 0:
        print("\n▼差（下げ - 上げ）")
        print((down_df.mean(numeric_only=True) - up_df.mean(numeric_only=True)).sort_values(ascending=False))

    compare_df = pd.DataFrame({
        "down_mean": down_df.mean(numeric_only=True),
        "up_mean": up_df.mean(numeric_only=True),
        "diff_down_minus_up": down_df.mean(numeric_only=True) - up_df.mean(numeric_only=True),
    }).reset_index().rename(columns={"index": "feature"})

    compare_path = Path(r"C:\kabu_trade\compare_break_retest_fail.csv")
    compare_df.to_csv(compare_path, index=False, encoding="utf-8-sig")
    print(f"比較CSV保存: {compare_path}")
    ...
    filters = {
        "no_filter": cond,
        "ma20_only": cond & (df["ma20_slope"] > -20),
        "ma20_ma50": cond & (df["ma20_slope"] > -20) & (df["ma50_slope"] > -10),
    }

    for name, f in filters.items():
        total = int(f.sum())
        down = int((f & down_hit).sum())
        up = int((f & up_hit).sum())
        ...
        print(f"\n{name}")
        print(f"発生: {total}")
        print(f"下げ: {down} ({down_rate:.2f}%)")
        print(f"上げ: {up} ({up_rate:.2f}%)")
