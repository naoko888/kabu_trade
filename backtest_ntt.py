...
def add_indicators(df):
    ...
    ts_jst = ts + pd.Timedelta(hours=9)
...
def backtest(df, stop, tp, lot=100):
    trades = []; day_pnl = {}; week_pnl = {}; day_consec = {}
    ...
    for i in range(len(df)):
        ...
        for j in range(i+1, last_bar+1):
            ...
            hhmm_j = int(df.iloc[j]["hhmm_jst"]); exit_time = df.index[j]
            if 1125 <= hhmm_j <= 1130:
                ep = float(df.iloc[j]["close"])
                result_trade = {"result":"LUNCH","exit":ep,"pnl":(ep-entry_price)*lot,"exit_time":exit_time}; break
            if h >= entry_price + tp:
                result_trade = {"result":"TP","exit":entry_price+tp,"pnl":float(tp)*lot,"exit_time":exit_time}; break
            if l <= entry_price - stop:
                result_trade = {"result":"SL","exit":entry_price-stop,"pnl":float(-stop)*lot,"exit_time":exit_time}; break
        if result_trade is None:
            ep = float(df.iloc[last_bar]["close"])
            result_trade = {"result":"TIME","exit":ep,"pnl":(ep-entry_price)*lot,"exit_time":df.index[last_bar]}
        pnl = result_trade["pnl"]
        day_pnl[date]      = day_pnl.get(date, 0) + pnl
        week_pnl[week_key] = week_pnl.get(week_key, 0) + pnl
        if pnl < 0: day_consec[date] = day_consec.get(date, 0) + 1
        else: day_consec[date] = 0
        if day_pnl[date] <= -MAX_LOSS_DAY: skip_days.add(date)
        if week_pnl[week_key] <= -MAX_LOSS_WEEK: skip_weeks.add(week_key)
        trades.append({"entry_time":df.index[i+1],"exit_time":result_trade["exit_time"],
                       "entry":entry_price,"exit":result_trade["exit"],
                       "pnl":pnl,"result":result_trade["result"]})
...
def optimize_rr(df):
    print("\n" + "="*65)
    print("【RR最適化】STOP x TP パターン探索（100株）")
    print("="*65)
    stops = [1, 2, 3, 4, 5]
    tps   = [2, 3, 4, 5, 6, 8, 10]
    results = []
    for stop in stops:
        for tp in tps:
            if tp <= stop: continue
            _, s = backtest(df, stop=stop, tp=tp, lot=100)
            if s and s["n"] >= 5:
...
if __name__ == "__main__":
    ...
    if rdf is not None and len(rdf) > 0:
        ...
        print(f"\n【株数別シミュレーション】STOP={stop_best}円 / TP={tp_best}円")
        print(f"{'株数':>6} | {'月平均':>9} | {'月最低':>9} | {'最大DD':>9}")
        print("-"*45)
        for shares in [100, 200, 300, 500]:
            _, s = backtest(df, stop=stop_best, tp=tp_best, lot=shares)
            if s:
                print(f"{shares:>5}株  | {s['monthly_avg']:>+9,.0f}円 | "
                      f"{s['monthly_min']:>+9,.0f}円 | {s['max_dd']:>+9,.0f}円")
        trades, _ = backtest(df, stop=stop_best, tp=tp_best, lot=100)
        if trades is not None:
            trades.to_csv("backtest_ntt.csv", index=False, encoding="utf-8-sig")
            print("\nbacktest_ntt.csv に保存しました")