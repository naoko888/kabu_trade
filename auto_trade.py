...
# ===== ①③④⑤合算DD管理 =====
ALL_DD_LIMIT_YEN = -30_000   # 全系統合算月次DD上限（円）
...
def check_sys5_signal(df):
    ...
    条件：
    - bar開始時刻のhourがS5_HOURS_DST/WIN（DST判定）に含まれる
...
#
# 系統①: long  月火水 × DST:(8,15,18,19,20,21) / 冬:(8,12,15,18,20,21,23) × 3月・5月・11月除外 / CPI除外なし
...
def check_micro_signal(df):
    ...
    if above_ma and touch_lo and gc:
        ...
        #         fired.append("②")
        # 系統①: 月火水 × DST:(2,8,15,18,19,21) / 冬:(2,8,12,13,15,18,21,23) × 3月・5月・11月除外（7月復活）
    ...
    if below_ma and touch_hi and dc:
        ...
        # 5月は後半（16日以降）のみエントリー
        _s3_may_ok = True
        if wd in (0, 2, 3, 4) and month not in (7, 11) and _s3_may_ok:
        ...
        if reason:
            ...
            if not MICRO_DRY_RUN:
                ...
                if reason == "TP到達":
                    ...
                    # 強制決済（夜間終了・金曜夜間・土曜）: SL・TP両方キャンセル → 成行決済
                    sl_oid = pos.get("sl_order_id")
                    if sl_oid:
...
def check_micro_entry(now, micro_board):
    ...
    global sys4_last_signal_bar_time, sys5_last_signal_bar_time
    global all_monthly_pnl_yen, all_monthly_stopped, all_monthly_current_month
