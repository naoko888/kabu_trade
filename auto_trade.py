...
# ===== 系統⑤：逆張りショート =====
SYS5_MOVE_PCT      = 0.003    # 直前3本比 +0.3%以上の上昇
SYS5_RSI_TH        = 70       # RSI14 >= 70
SYS5_VOL_TH        = 0.8      # vol_ratio >= 0.8
SYS5_LOOKBACK      = 3        # 何本前との比較か
SYS5_RECOVERY_PCT  = 0.002    # ひげ戻り率 >= 0.2%（(high-close)/close）
SYS5_TP            = 120      # 利確幅（pt）
SYS5_SL            = 60       # 損切幅（pt）
SYS5_MAX_HOLD      = 6        # 最大保有足数

# ===== 系統④⑤合算DD管理 =====
SYS45_DD_LIMIT_YEN = -3000    # ④⑤合算月次DD上限（円）。超えたら⑤のみ停止。
...
# 系統⑤ 状態管理
sys5_last_signal_bar_time = None   # 重複エントリー防止用

# 系統④⑤合算 DD管理
sys45_monthly_pnl_yen = 0.0   # ④⑤合算 月次累積損益（円）。毎月1日リセット。
sys45_stopped         = False  # True=今月の④⑤合算DD上限到達で⑤停止中
sys45_current_month   = None   # 現在の年月 (year, month)。月次リセット用。
...
def check_sys5_signal(df):
    """系統⑤：逆張りショート シグナル判定
    - 直前SYS5_LOOKBACK本比でSYS5_MOVE_PCT以上の上昇
    - RSI14 >= SYS5_RSI_TH
    - vol_ratio >= SYS5_VOL_TH
    - ひげ戻り率 >= SYS5_RECOVERY_PCT（(high-close)/close）
    - 時間除外なし・CPI除外なし
    """
    ...
    move_pct = (cur["close"] - prev["close"]) / prev["close"]
    recovery = (cur["high"] - cur["close"]) / cur["close"] if cur["close"] != 0 else 0

    return all([
        move_pct >= SYS5_MOVE_PCT,
        cur["rsi14"] >= SYS5_RSI_TH,
        cur["vol_ratio"] >= SYS5_VOL_TH,
        recovery >= SYS5_RECOVERY_PCT,
    ])
...
def _monitor_micro_dry_inner(now, hhmm, micro_board, verbose):
    ...
    for pos in micro_dry_positions:
        ...
        if reason:
            ...
            # ── 系統④⑤合算 月次DD更新 ──
            if sys_label in ("④", "⑤"):
                global sys45_monthly_pnl_yen, sys45_stopped
                trade_yen_45 = round(pnl * MICRO_PT_TO_YEN - MICRO_COMMISSION_YEN, 0)
                sys45_monthly_pnl_yen += trade_yen_45
                log(f"[系統④⑤] 合算月次損益:{sys45_monthly_pnl_yen:,.0f}円  DD上限:{SYS45_DD_LIMIT_YEN:,.0f}円")
                if sys45_monthly_pnl_yen <= SYS45_DD_LIMIT_YEN and not sys45_stopped:
                    sys45_stopped = True
                    log(f"[系統④⑤] 合算DD上限到達({sys45_monthly_pnl_yen:,.0f}円) → 今月の系統⑤を停止")
...
def check_micro_entry(now, micro_board):
    ...
    global sys5_last_signal_bar_time
    global sys45_monthly_pnl_yen, sys45_stopped, sys45_current_month
    ...
    # ── 系統④⑤合算 月次リセット ──
    if sys45_current_month != now_ym:
        sys45_current_month   = now_ym
        sys45_monthly_pnl_yen = 0.0
        sys45_stopped         = False
        log(f"[系統④⑤] 月次リセット: {now_ym} 損益リセット・⑤停止解除")
    ...
    # ===== 系統⑤：逆張りショート判定 =====
    if sys45_stopped:
        log(f"[系統⑤] ④⑤合算DD上限到達のため停止中 (合算DD:{sys45_monthly_pnl_yen:,.0f}円)")
